...
from arbiter.policy.schema import PolicyContext, PolicyDefinition
...
def lint_policy(policy: PolicyDefinition) -> list[LintFinding]:
    ...
    if policy.default_action == Action.ALLOW:
        findings.append(LintFinding(
            level="WARNING",
            message=f"Policy default_action is set to 'allow'. It is strongly recommended to fail-closed with 'ask' or 'deny'."
        ))
