...
from jevfuse.schema.internal import RawScore


class CacheEntry(BaseModel):
    """Cached decision raw score or payload with TTL metadata."""
    input_hash: str = Field(..., description="SHA-256 fingerprint")
    raw_score: RawScore | dict[str, Any] | Any = Field(..., description="Cached model output")
...
class CacheStore(Protocol):
    ...
    async def set(
        self,
        input_hash: str,
        raw_score: RawScore | dict[str, Any] | Any,
        provider: str,
